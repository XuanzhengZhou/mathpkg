...
INPUT_FILE = '/home/a123/文档/mathpkg/pipeline_output/dedup/canonical_with_distance.json'
LEAN_PROJ = '/home/a123/文档/mathpkg/lean/MathPkg'
DONE_DIR = '/home/a123/文档/mathpkg/pipeline_output/skel_done'
...
def main():
    ...
    # Get concept — filter to distance=1 subset, THEN index
    data = json.load(open(INPUT_FILE))
    d1 = [c for c in data if c.get('mathlib4_distance') == 1]
    c = d1[idx]
    cid = c['id']
    lean_file = os.path.join(LEAN_PROJ, 'MathPkg', 'Skeleton', f'{cid}.lean')
    ...
    # Compile
    cmd = f'cd {LEAN_PROJ} && export PATH="$HOME/.elan/bin:$PATH" && timeout 30 lake env lean MathPkg/Skeleton/{cid}.lean 2>&1'
