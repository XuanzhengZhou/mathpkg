...
(b) Use (a) and the functional equation of $\vartheta$ to obtain the representation
$$\pi^{-s/2} \Gamma\left(\frac{s}{2}\right) \zeta(s) = \frac{1}{s(s-1)} + \int_{1}^{\infty} \left(x^{s/2-1} + x^{(1-s)/2-1}\right) \psi(x) dx$$
for $\sigma > 1$.

(c) Show that the equation in (b) gives the analytic continuation of $\zeta(s)$ beyond the line $\sigma = 1$ and that it also implies the functional equation stated above.
