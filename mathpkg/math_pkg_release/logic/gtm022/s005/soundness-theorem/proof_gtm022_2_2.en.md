...
role: proof
locale: en
of_concept: soundness-theorem-prop-calculus
source_book: gtm022
source_chapter: "II"
...
Let $v: P(X) \to \mathbb{Z}_2$ be a valuation for which $v(A) \subseteq \{1\}$. We shall use induction over the length $n$ of the proof of $p$ from $A$ to show that $v(p) = 1$.

Suppose that $n = 1$. Then $p \in A \cup \mathcal{A}$, and since every axiom is a tautology, we have $v(p) = 1$.
