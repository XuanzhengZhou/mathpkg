...
role: proof
locale: en
of_concept: even-subset-is-subspace
source_book: gtm054
source_chapter: "2"
source_section: "IIA"
---
...
Select $x_0 \in U$. One can easily verify that if $\mathcal{B} = \{\{x_0, x\}: x \in U \setminus \{x_0\}\}$, then $\mathcal{B}$ is independent and $|\mathcal{B}| = |U| - 1$. Hence $\dim(\mathcal{E}(U)) \geq |U| - 1$. However, since $U \neq \varnothing$, $\mathcal{E}(U) \neq \mathcal{P}(U)$, so $\dim(\mathcal{E}(U)) < \dim(\mathcal{P}(U)) = |U|$. Therefore $\dim(\mathcal{E}(U)) = |U| - 1$.
