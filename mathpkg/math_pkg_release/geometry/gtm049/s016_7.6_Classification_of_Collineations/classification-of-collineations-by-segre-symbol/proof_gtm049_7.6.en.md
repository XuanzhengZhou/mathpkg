...
**Proof.** (i) Suppose the Segre symbols are the same and let the elementary divisors of $f, g$ be $(X - x_i)^{n_{ij}}, (X - y_i)^{n_{ij}}$, respectively. Suppose
...
(ii) Since $f$ and $e^{-1}f e$ have the same Segre symbol, the converse follows immediately from (i) by taking $g = e^{-1}f e$ and noting that $\mathcal{P}(e)$ maps the invariant configuration of $\mathcal{P}(f)$ onto that of $\mathcal{P}(g)$.
