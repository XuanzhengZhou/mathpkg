...
is analytic in an open set containing $\bar{B}(0; r)$, has no zeros in $B(0; r)$, and $|F(z)| = |f(z)|$ for $|z| = r$. Applying the Mean Value Property for harmonic functions (equation (1.1)) to $F$ gives
...
$$
F(0) = f(0) \prod_{k=1}^{n} \left( -\frac{r}{a_k} \right),
$$

so that

$$
\log |F(0)| = \log |f(0)| + \sum_{k=1}^{n} \log \frac{r}{|a_k|}.
$$

Substituting this into the integral equality yields Jensen's Formula.
