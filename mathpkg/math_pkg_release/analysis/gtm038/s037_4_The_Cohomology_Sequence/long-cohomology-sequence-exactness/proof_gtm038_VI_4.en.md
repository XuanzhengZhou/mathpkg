...
Since the canonical resolution $\mathfrak{W}(\mathcal{S})$ is exact and $\mathfrak{W}(\mathcal{S}^{**})$ is exact, and since $(W_{i-1}\psi)_*$ is surjective, there exists $\eta \in \Gamma(X, \mathcal{S}_{i-1})$ with $(W_{i-1}\psi)_*(\eta) = \eta_0$.
...
Then $d\sigma = 0$ (since $d\xi = 0$ and $dd\eta = 0$). Therefore $\sigma \in Z^i(X, \mathcal{S}^*)$ and
...
2. If $\xi^{**} \in Z^i(X, \mathcal{S}^{**})$ and $\partial(q_i^{**}\xi^{**}) = 0$, then by construction of $\partial$ and the exactness properties of the canonical double complex, there exist $\eta_0 \in \Gamma(X, \mathcal{S}_{i-1})$ and $\xi \in \Gamma(X, \mathcal{S}_i)$ such that $(W_i\psi)_*(\xi) = d\eta_0$ and the corresponding cohomology class satisfies the required relation.

More precisely, from $\partial(q_i^{**}\xi^{**}) = 0$ and the surjectivity of $(W_{i-1}\psi)_*$, one obtains $\eta = d\sigma$ for some $\sigma$, and
