...
If $\{x_{\gamma}\}_{\gamma \in \Gamma}$ is a summable family of vectors in a Banach space $\mathcal{E}$, and if $\Gamma'$ is any subset of $\Gamma$, then $\{x_{\gamma}\}_{\gamma \in \Gamma'}$ is also summable. Show also that, if we set $\Gamma'' = \Gamma \setminus \Gamma'$, then
