...
write_concept() {
    local dir="$1" slug="$2" type="$3" title="$4" domain="$5" subdomain="$6" difficulty="$7" deps="$8" tex="$9" intro="${10}"
    local d="$dir/$slug"
    mkdir -p "$d"
    # concept.yaml
    cat > "$d/concept.yaml" << YAMLEOF
id: "$slug"
title_en: "$title"
title_zh: ""
type: $type
...
source_books: [{book_id: "gtm033", author: "Morris W. Hirsch", title: "Differential Topology", chapter: "", section: "", pages: "", role_in_book: ""}]
content_hash: ""
...
# =========== s012 ===========
D12="$BASE/s012_6_Collars_and_Tubular_Neighborhoods_of_Neat_Submanifolds"
...
# =========== s014 ===========
D14="$BASE/s014_1_Degrees_of_Maps"
mkdir -p "$D14"
...
$$\omega_u/\kappa_u = (\partial \omega)_u \Longleftrightarrow \omega_v/\kappa_v = -(\partial \omega)_v.$$' \
  "This lemma describes how the orientation of the normal bundle of an arc relates to the boundary orientation at the two endpoints. If the quotient orientation agrees with the boundary orientation at one endpoint, it must be opposite at the other endpoint. This is used to track signs when computing degrees."
...
write_concept "$D14" "lemma-1-5-smooth-approximation" "lemma" \
  ...
  'Let $M, N$ be manifolds and $f: M \to N$ a continuous map. Then $f$ can be approximated by $C^\infty$ maps homotopic to $f$.' \
...
# =========== s016 ===========
D16="$BASE/s016_1_Morse_Functions"
mkdir -p "$D16"
...
# =========== s017 ===========
D17="$BASE/s017_2_Differential_Equations_and_Regular_Level_Surfaces"
mkdir -p "$D17"
...
write_concept "$D17" "theorem-2-2-regular-interval-theorem" "theorem" \
  ...
  'Let $f: M \to [a, b]$ be a $C^{r+1}$ map on a compact $\partial$-manifold, $1 \leqslant r \leqslant \omega$. Suppose $f$ has no critical points and $f(\partial M) = \{a, b\}$. Then there is a $C^r$ diffeomorphism $F: f^{-1}(a) \times [a, b] \to M$ such that the diagram
$$\begin{CD} f^{-1}(a) \times [a, b] @>F>> M \\ @V\pi VV @VVfV \\ [a, b] @= [a, b] \end{CD}$$
commutes. In particular all level surfaces of $f$ are diffeomorphic.' \
...
# =========== s018 ===========
D18="$BASE/s018_3_Critical_Levels_and_CW"
mkdir -p "$D18"
...
# =========== s021 ===========
D21="$BASE/s021_3_Isotopies_of_Disks"
mkdir -p "$D21"
...
write_concept "$D21" "theorem-3-2-isotopy-of-disjoint-disks" "theorem" \
  ...
  "Two pairs of disjoint embedded $n$-disks in a connected $n$-manifold are simultaneously isotopic, provided the orientation conditions match. This generalizes Theorem 3.1 to multiple disks and is used in constructing connected sums and analyzing handle decompositions."
...
write_concept "$D21" "theorem-1-8-crosscap-on-orientable-surface" "theorem" \
  ...
  "Adding a crosscap (taking connected sum with the real projective plane) to an orientable surface of genus $p$ produces a nonorientable surface of genus $2p+1$. This relation together with Theorem 1.7 forms the basis for the complete classification of surfaces."
...
echo "All concepts created successfully!"
