...
role: proof
locale: en
of_concept: prolongation-of-principal-h-bundle-to-g-bundle
source_book: gtm020
source_chapter: "6"
source_section: "2"
---
...
Conversely, a cross section $\sigma$ of $\xi[G \bmod H]$ defines a map $g: X \rightarrow G \bmod H$ such that
$$
g(xs) = s^{-1}g(x) = g(x)s
$$
for each $s \in G$ by Theorem 4(8.1). Let $Y$ be the closed subset $g^{-1}(eH)$ of $X$, let $q$ be the restriction $p|Y$, and let $\eta = (Y, q, B)$. Let $y_1, y_2 \in Y$ such that $q(y_1) = q(y_2)$. For some $s \in G$ we have $y_2 = y_1 s$,
$$
eH = g(y_2) = g(y_1 s) = s^{-1}g(y_1) = s^{-1}eH,
$$
and $s \in H$. Finally, the restriction of the translation function of the principal $G$-space $X$ is the translation function of the principal $H$-space $Y$.
