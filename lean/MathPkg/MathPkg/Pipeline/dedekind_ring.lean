...
/-- Every principal ideal domain (PID) is a Dedekind ring. -/
example (R : Type*) [CommRing R] [IsDomain R] [IsPrincipalIdealRing R] :
    DedekindRing R := by
  infer_instance

/-- The ring of integers `𝓞 K` of a number field `K` is a Dedekind ring. -/
example (K : Type*) [Field K] [NumberField K] : DedekindRing (𝓞 K) := by
  infer_instance

/-- In a Dedekind ring, every nonzero ideal has a unique factorization into prime
ideals. This is expressed in Mathlib4 via the `UniqueFactorizationMonoid` instance
on `Ideal R`. -/
example (R : Type*) [CommRing R] [IsDomain R] [DedekindRing R] :
    UniqueFactorizationMonoid (Ideal R) := by
  infer_instance

/-- A concrete factorization: in `ℤ`, the ideal `(6)` equals `(2) * (3)`, both prime. -/
...
/-- In a Dedekind ring, every nonzero prime ideal is maximal (Krull dimension ≤ 1). -/
example (R : Type*) [CommRing R] [IsDomain R] [DedekindRing R]
    (p : Ideal R) (hp_ne : p ≠ ⊥) (hp_prime : p.IsPrime) : p.IsMaximal :=
  hp_prime.isMaximal hp_ne
