...
-/
abbrev Uniformizer (u : R) : Prop := Irreducible u
...
-/
example (v : Valuation R ℤₘ₀) [IsRankOneDiscrete v] (u : R) : Prop :=
  v.IsUniformizer u

/--
**Example** (DVR context): In `ℤ₍ₚ₎` (the p-adic integers) or any DVR, an irreducible element
serves as a uniformizer.

For instance, in `ℤ₍ₚ₎` the prime `p` is a uniformizer: `addVal ℤ₍ₚ₎ p = 1`.
-/
example (R : Type*) [CommRing R] [IsDomain R] [IsDiscreteValuationRing R] (ϖ : R) (h : Irreducible ϖ) :
    addVal R ϖ = 1 :=
  addVal_uniformizer h
